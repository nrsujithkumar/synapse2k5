...
//                 </div>
//                 <motion.div className="mt-10 flex justify-center items-center gap-4" initial={{ opacity: 0, y: 20 }} animate={showButtons ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2, ease: "easeOut" }} style={{ transform: 'translateZ(40px)' }}>
//                     <motion.button className="px-8 py-3 bg-sky-500 text-white font-semibold rounded-lg shadow-lg shadow-sky-500/30 hover:shadow-sky-500/50 transition-shadow" whileHover={{ scale: 1.05, y: -3 }} whileTap={{ scale: 0.95 }} transition={{ type: 'spring', stiffness: 300, damping: 15 }}>Get Started</motion.button>
...
// 1. The component now accepts a 'setView' function as a prop.
    ...
    return (
        <section ref={ref} id="home" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className="relative min-h-screen flex flex-col justify-center items-center text-center overflow-hidden" style={{ perspective: '2000px' }}>
            <motion.div className="container mx-auto px-4 sm:px-6 z-10" style={{ rotateX, rotateY, transformStyle: 'preserve-3d' }}>
                ...
                >
                    {/* 2. When this button is clicked, it calls the setView function */}
                    <motion.button
                        onClick={() => setView('signup')}
                        className="px-8 py-3 bg-sky-500 text-white font-semibold rounded-lg shadow-lg shadow-sky-500/30 hover:shadow-sky-500/50 transition-shadow"
                    ...
                    {/* 3. This button will navigate to the login view (which we'll build in Phase 7) */}
                    <motion.button
                        onClick={() => setView('login')}
                        className="px-8 py-3 bg-gray-500/10 dark:bg-gray-200/5 text-gray-800 dark:text-gray-300 font-semibold rounded-lg backdrop-blur-sm border border-gray-500/10"
                        whileHover={{ scale: 1.05, y: -3, backgroundColor: 'rgba(107, 114, 128, 0.2)' }}
                        whileTap={{ scale: 0.95 }}
                        transition={{ type: 'spring', stiffness: 300, damping: 15 }}
                    >
                        Login
